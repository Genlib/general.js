/* general.js functions  */
	   ...
	   function Page_Number(obj){
		      ...
		      this.write_item_number = function(current_index){
				 ...
				 document.write(this.set_item_number(current_index));
		      } // end
				  ...
				  for(var smi=0;smi<selected_values.length;smi++)
					{
							...
							select_match(selected_values[smi],element);
			  		}
	    }
...
function crypt_url(url){	
   ...
   document.location.href=(q_index==-1)?url+'?PASS_ID='+sess_id:url+'&'+'PASS_ID='+sess_id;
	...
	// Trimming the content * make extra a  mouse over option
	   ...
	   function trim_and_tip(content,trim_length){
				 ...
				 if(content.length>trim_length){
					    ...
					    document.write('<a  data-hint="'+content+'"  class="hint--bottom ">'+content.substr(0,trim_length)+'..</a>');
				 }else{
				 	    document.write(content);
				 }
...
///////////////////////  FORM GENERAL VALUS ////////////////////////////////////////////
	...
	function check_all(element_count,element_prefix){
			...
			element_prefix=(element_prefix)?element_prefix:'c';
			
	
			for(var i=1;i<=element_count;i++){   
							
														
			   	 D.getElementById(element_prefix+i).checked=true;				 
			}
	}
	...
	function clear_all(element_count,element_prefix){ 
			...
			for(var i=1;i<=element_count;i++)	{   D.getElementById(element_prefix+i).checked=false;} 
	}
      ...
      function add_focus_style(param){
	   ...
	   function Option_Builder_Csv(obj){
	    ...
	    function Option_Builder_Series(obj){
		      ...
		      for (var opt_index=obj.start;opt_index<obj.end;opt_index++){				 		
				 
				 var select_option = document.createElement("OPTION");
					       
				 select_option.text=opt_index;
					       
				 select_option.value=opt_index;
	   ...
	   General.prototype.write_decimal = function(price){
		   ...
		   document.write(this.price_symbol+this.set_decimal(price));    
	   }
